...
contract user{
  ...
  function rideReq(string memory _source, string memory _destination, string memory _date, string memory _time) public {
    ...
    users[reqCount] = User(reqCount, _source, _destination, 0 ,_date,_time, payable(msg.sender));

    emit UserCreated(reqCount, _source, _destination, 0 ,_date,_time , payable(msg.sender));
  }
/*
  event ImageTipped(
    uint id,
  ...
  function uploadImg(string memory _imgHash, string memory _name,bool _verified) public {
    ...
    driver[imgCount] = Driver(imgCount, _imgHash, _name, 0 ,_verified,veri_fy,  payable(msg.sender));

    emit DiverCreated(imgCount, _imgHash, _name, 0 ,veri_fy,  payable(msg.sender));
  }
...
/*
  ...
  function uploadRide(string memory _rsource, string memory _rdestination, string memory _rdate, string memory _rtime, string memory _ride, uint amount) public {
    ...
    ride[rideCount] = Ride(rideCount, _rsource, _rdestination, _rdate ,_rtime,_ride,amount, payable(msg.sender));

    emit RideCreated(rideCount, _rsource, _rdestination, _rdate ,_rtime,_ride,amount, payable(msg.sender));
  }
